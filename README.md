...
Use `orc plan "<task>"` when you want to shape a broad or high-risk consultation first. Explicit planning has two named loops: Augury Interview runs before the planning spec when operator clarification is needed, and Plan Conclave runs after the planning spec to let architect/critic review revise the consultation-plan draft before candidate generation. Augury records an `ontologySnapshot` sign bundle: goals, constraints, non-goals, acceptance criteria, and risks that future candidates must satisfy or violate. `interviewDepth` controls Augury Interview and also acts as a bounded modifier for the Plan Conclave budget; it does not add interview rounds to Plan Conclave. Oraculum re-evaluates depth and intensity on Augury continuation answers, derives the effective Plan Conclave revision budget from `consensusReviewIntensity` plus `interviewDepth`, then clamps it to the operator cap. If the request is still unclear, Oraculum asks one concrete clarification and records `planning-depth.json` plus `planning-interview.json`; answer with another `orc plan "<answer>"` and Oraculum connects it to the active interview when the runtime classifies it as a continuation. When ready, it persists `planning-spec.json`, `plan-consensus.json`, a reusable `consultation-plan.json`, `plan-readiness.json`, and a human-readable `consultation-plan.md`; you can later run `orc consult <plan-artifact>`. If Plan Conclave rejects the draft or misses the revision cap, planning stops before candidate generation with `approved=false` and the required changes are shown as Plan Conclave remediation; answer with another `orc plan "<answer>"` and Oraculum creates a new run linked to the blocked consensus when the runtime classifies the input as remediation. Review-runtime-unavailable blockers are not answerable remediation and should be rerun when review can execute.
