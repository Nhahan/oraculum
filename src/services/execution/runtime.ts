...
export async function executeRun(options: ExecuteRunOptions): Promise<ExecuteRunResult> {
  ...
  await options.onProgress?.(
    candidatesLaunchingEvent(specSearch.implementationCandidateIds.length),
  );

  let { candidateMap, executionRecords, scorecardsByCandidate, selectionMetrics } =
    await executeInitialCandidates({
      adapter,
      candidateIdsToExecute: specSearch.implementationCandidateIds,
      ...(consultationPlan ? { consultationPlan } : {}),
  ...
  if (!hasPromotedCandidate(candidateMap) && specSearch.backupCandidateIds.length > 0) {
    const [backupCandidateId] = specSearch.backupCandidateIds;
