...
export function buildClarifyFollowUpPrompt(request: AgentClarifyFollowUpRequest): string {
  const sections: string[] = [
    ...
    'Return JSON only in this shape: {"summary":"short rationale","keyQuestion":"one short question","missingResultContract":"one concrete missing blocking contract or evidence statement","missingJudgingBasis":"one concrete missing judging-basis statement"}',
    "",
  ...
  sections.push(
    "",
    "Rules:",
    "- Keep one bounded keyQuestion only, tied to the repeated blocker.",
    "- keyQuestion must be answerable by the operator without repository inspection, command flags, or implementation details.",
    "- missingResultContract must state the exact missing blocking contract or evidence that still blocks safe candidate generation.",
