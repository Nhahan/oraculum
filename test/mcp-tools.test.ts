...
const mockedHasNonEmptyTextArtifact = vi.mocked(hasNonEmptyTextArtifact);
...
describe("chat-native MCP tools", () => {
  ...
  it("keeps inspect-comparison-report in verdict status when only comparison json is available", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-json-only-comparison-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("keeps inspect-comparison-report in verdict status when json is malformed but markdown is valid", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-markdown-fallback-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("omits direct crown from verdict status when second-opinion is unavailable", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-second-opinion-unavailable-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("keeps manual review explicit when a crowning record and second-opinion blocker both exist", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-crowning-manual-review-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("returns aligned review, artifacts, and status when a crowned recommendation still requires manual review", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-crowned-manual-review-response-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("returns aligned review, artifacts, and status when an uncrowned recommendation requires manual review", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-uncrowned-manual-review-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
  ...
  it("does not reintroduce comparison inspection when a crowned recommendation still requires manual review", async () => {
    const root = await mkdtemp(join(tmpdir(), "oraculum-mcp-crowned-manual-review-comparison-"));
    tempRoots.push(root);
    const manifest = createCompletedManifest();
    mockedReadRunManifest.mockResolvedValue({
      ...manifest,
