...
describe("run execution oracles: path and environment policy", () => {
  it(
    "builds oracle PATH from existing local tool directories only",
    async () => {
      ...
      await writeExecutionTask(cwd, "local-tool-paths.md", "# Local tool paths\nCheck PATH.\n");
  ...
  it(
    "orders candidate-local oracle PATH entries before project-root local tools",
    async () => {
      const cwd = await createInitializedExecutionProject();
      const fs = await import("node:fs/promises");
      await fs.mkdir(join(cwd, "node_modules", ".bin"), { recursive: true });
      await configureProjectOracles(cwd, [
        {
      ...
      await writeExecutionTask(
        cwd,
        "candidate-tool-path-precedence.md",
  ...
  it(
    "preserves an explicit empty oracle PATH override over local tool directory injection",
    async () => {
      const cwd = await createInitializedExecutionProject();
      await configureProjectOracles(cwd, [
        {
      ...
      await writeExecutionTask(cwd, "empty-path.md", "# Empty PATH\nCheck env override.\n");
  ...
  it(
    "does not inherit global oracle PATH unless the oracle opts in",
    async () => {
      const cwd = await createInitializedExecutionProject();
      ...
      await writeExecutionTask(cwd, "path-policy.md", "# PATH policy\nCheck oracle PATH.\n");
  ...
  it(
    "does not leak unrelated host environment variables into repo-local oracles",
    async () => {
      const cwd = await createInitializedExecutionProject();
      await configureProjectOracles(cwd, [
        {
      ...
      await writeExecutionTask(cwd, "host-env-isolation.md", "# Host env isolation\nCheck env.\n");
